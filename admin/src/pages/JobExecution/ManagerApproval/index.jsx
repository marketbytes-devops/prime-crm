...
const ManagerApproval = ({ handlePrint }) => {
  const [workOrders, setWorkOrders] = useState([]);
  const [selectedWorkOrder, setSelectedWorkOrder] = useState(null);
  const [showApprovalModal, setShowApprovalModal] = useState(false);
  const [approvalData, setApprovalData] = useState({ work_order_id: null, approve: false, comment: "" });
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 20;

  useEffect(() => {
    apiClient.get("work-orders/?current_status=Pending Approval")
  ...
  const handleApprovalAction = (wo, approve) => {
    setApprovalData({ work_order_id: wo.id, approve, comment: "" });
    setShowApprovalModal(true);
  };

  const handleSubmitApproval = () => {
    if (!approvalData.work_order_id) return;
    apiClient.post("manager-approval/", approvalData)
      .then(response => {
        setWorkOrders(prev => prev.filter(wo => wo.id !== approvalData.work_order_id));
        toast.success(`Work order ${approvalData.approve ? "approved" : "declined"} successfully!`);
        setShowApprovalModal(false);
        setApprovalData({ work_order_id: null, approve: false, comment: "" });
      })
      .catch(error => toast.error("Failed to process approval: " + (error.response?.data?.error || error.message)));
  };
  ...
  return (
    <div className="p-4">
      <h2 className="text-xl font-semibold mb-4 text-black">Manager Approval</h2>
      ...
      {showApprovalModal && (
        <div className="fixed inset-0 backdrop-brightness-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-sm p-6 w-full max-w-md">
            <h3 className="text-lg font-semibold mb-3 text-black border-b pb-2">
              {approvalData.approve ? "Approve" : "Decline"} Work Order
            </h3>
            {!approvalData.approve && (
              <div className="mb-4">
            ...
            <div className="flex justify-end space-x-2">
              <button
                ...
                className={`${
                  approvalData.approve ? "bg-green-500 hover:bg-green-600" : "bg-red-500 hover:bg-red-600"
                } text-white px-3 py-2 rounded flex items-center`}
              >
                {approvalData.approve ? <CheckCircle size={16} className="mr-1" /> : <XCircle size={16} className="mr-1" />}
                {approvalData.approve ? "Approve" : "Decline"}
              </button>
              <button
